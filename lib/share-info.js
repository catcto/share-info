        ...
        var req = self.request(opts, function (error, response) {
            ...
            if (error) {
                if (opts.retries) {
                    ...
                    opts.callback(error, results, callback);
                }
            } else {
                if (response.statusCode === 200) {
                    ...
                    opts.callback(new Error('ESTATUSCODE:' + response.statusCode), results, callback);
                }
            }
            ...
            if (!/text\/html/i.test(response.headers['content-type'])) {
                req.abort();
                opts.callback(new Error('ECONTENTTYPE:' + response.headers['content-type']), results, callback);
            }
        });
    };
    doReq();
